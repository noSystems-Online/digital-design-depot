...
serve(async (req) => {
  ...
  try {
    ...
    const { amount, currency, description, billingInfo, items, config } = await req.json()

    console.log('Creating PayPal order:', { amount, currency, environment: config.environment });
